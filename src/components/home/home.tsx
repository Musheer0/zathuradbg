...
<div className="hidden lg:block absolute h-full w-2 bg-white right-40 origin-top blur-xl rotate-45 fadeinhalf"></div>


   <p className="text-sm overflow-hidden  relative text-indigo-200 px-4  rounded-full py-3 mb-5 mt-20 border border-zinc-900 bg-zinc-950">
    <span className="p-1 relative rounded-full bg-indigo-700 text-black font-semibold text-xs mr-2">NEW</span>
     {announcment}
     <span className="mix-blend-overlay slideout  absolute h-full w-2 top-0 left-0 bg-white  -rotate-12"></span>
     <span className="mix-blend-overlay slideout  absolute h-full w-2 top-0 left-0 bg-white  -rotate-12"></span>
     </p>
...
{main_title_sub}
   ...
   <div className="circle hidden sm:flex  absolute w-[900px] h-[900px] scale-[1.4] -top-1/3 opacity-5 -translate-x-1/2 left-1/2   border rounded-full "></div>
   <div className="circle hidden sm:flex absolute w-[900px] h-[900px] -top-1/3 opacity-5 -translate-x-1/2 left-1/2   border rounded-full "></div>
   <div className="circle hidden sm:flex absolute w-[900px] h-[900px] -top-1/3 scale-75 opacity-5 -translate-x-1/2 left-1/2   border rounded-full "></div>
    <div className="fadein absolute  w-full h-full top-0 rounded-full -translate-y-1/2 -translate-x-1/2 left-1/2 blur-3xl bg-indigo-600/10"></div>
    <div className="fadein absolute w-1/2 h-1/2 top-0 rounded-full -translate-y-1/2 -translate-x-1/2 left-1/2 blur-3xl mix-blend-lighten bg-indigo-600/20"></div>
    <div className="fadein absolute w-full h-full top-0 blur-3xl bg-indigo-600/40"></div>
